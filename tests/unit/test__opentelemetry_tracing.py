...
if HAS_OPENTELEMETRY_INSTALLED:
    ...
    class TestNoTracing(unittest.TestCase):
        def setUp(self):
            self._temp_opentelemetry = sys.modules[u"opentelemetry"]

            sys.modules[u"opentelemetry"] = None
            importlib.reload(_opentelemetry_tracing)

        def tearDown(self):
            sys.modules[u"opentelemetry"] = self._temp_opentelemetry
            importlib.reload(_opentelemetry_tracing)

        def test_no_trace_call(self):
            with _opentelemetry_tracing.trace_call(u"Test", _make_session()) as no_span:
                self.assertIsNone(no_span)

    class TestTracing(OpenTelemetryBase):
        def test_trace_call(self):
            extra_attributes = {
                u"attribute1": u"value1",
                # Since our database is mocked, we have to override the db.instance parameter so it is a string
                u"db.instance": u"database_name",
            }
            ...
            with _opentelemetry_tracing.trace_call(
                u"CloudSpanner.Test", _make_session(), extra_attributes
            ) as span:
                span.set_attribute(u"after_setup_attribute", 1)

            expected_attributes[u"after_setup_attribute"] = 1
            ...
            self.assertEqual(span.name, u"CloudSpanner.Test")
            self.assertEqual(span.status.canonical_code, StatusCanonicalCode.OK)

        def test_trace_error(self):
            extra_attributes = {u"db.instance": u"database_name"}
        ...
        def test_trace_grpc_error(self):
            ...
            expected_attributes = {
                u"db.type": u"spanner",
                u"db.url": u"spanner.googleapis.com:443",
                u"net.host.name": u"spanner.googleapis.com:443",
            }
            expected_attributes.update(extra_attributes)
        ...
        def test_trace_codeless_error(self):
            ...
            with self.assertRaises(GoogleAPICallError):
                with _opentelemetry_tracing.trace_call(
                    ...
                    raise GoogleAPICallError(u"error")
