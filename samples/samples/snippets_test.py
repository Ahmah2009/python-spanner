...
def unique_instance_id():
    u""" Creates a unique id for the database. """
...
def database(spanner_instance):
    u""" Creates a temporary database that is removed after testing. """
...
def test_read_only_transaction(capsys):
    ...
    assert out.count(u"SingerId: 1, AlbumId: 1, AlbumTitle: Total Junk") == 2


def test_add_timestamp_column(capsys):
    snippets.add_timestamp_column(INSTANCE_ID, DATABASE_ID)
    out, _ = capsys.readouterr()
    assert u'Altered table "Albums" on database ' in out
...
def test_query_data_with_timestamp(capsys):
    ...
    out, _ = capsys.readouterr()
    assert u"SingerId: 1, AlbumId: 1, MarketingBudget: 1000000" in out
    assert u"SingerId: 2, AlbumId: 2, MarketingBudget: 750000" in out


def test_create_table_with_timestamp(capsys):
    snippets.create_table_with_timestamp(INSTANCE_ID, DATABASE_ID)
    out, _ = capsys.readouterr()
    assert u"Created Performances table on database" in out
...
def test_dml_write_read_transaction(capsys):
    ...
    out, _ = capsys.readouterr()
    assert u"1 record(s) inserted." in out
    assert u"FirstName: Timothy, LastName: Campbell" in out
...
def test_query_data_with_array(capsys):
    ...
    out, _ = capsys.readouterr()
    assert u"VenueId: 19, VenueName: Venue 19, AvailableDate: 2020-11-01" in out
    assert u"VenueId: 42, VenueName: Venue 42, AvailableDate: 2020-10-01" in out


def test_query_data_with_bool(capsys):
    snippets.query_data_with_bool(INSTANCE_ID, DATABASE_ID)
    out, _ = capsys.readouterr()
    assert u"VenueId: 19, VenueName: Venue 19, OutdoorVenue: True" in out


def test_query_data_with_bytes(capsys):
    snippets.query_data_with_bytes(INSTANCE_ID, DATABASE_ID)
    out, _ = capsys.readouterr()
    assert u"VenueId: 4, VenueName: Venue 4" in out


def test_query_data_with_date(capsys):
    snippets.query_data_with_date(INSTANCE_ID, DATABASE_ID)
    out, _ = capsys.readouterr()
    assert u"VenueId: 4, VenueName: Venue 4, LastContactDate: 2018-09-02" in out
    assert u"VenueId: 42, VenueName: Venue 42, LastContactDate: 2018-10-01" in out


def test_query_data_with_float(capsys):
    snippets.query_data_with_float(INSTANCE_ID, DATABASE_ID)
    out, _ = capsys.readouterr()
    assert u"VenueId: 4, VenueName: Venue 4, PopularityScore: 0.8" in out
    assert u"VenueId: 19, VenueName: Venue 19, PopularityScore: 0.9" in out


def test_query_data_with_int(capsys):
    snippets.query_data_with_int(INSTANCE_ID, DATABASE_ID)
    out, _ = capsys.readouterr()
    assert u"VenueId: 19, VenueName: Venue 19, Capacity: 6300" in out
    assert u"VenueId: 42, VenueName: Venue 42, Capacity: 3000" in out


def test_query_data_with_string(capsys):
    snippets.query_data_with_string(INSTANCE_ID, DATABASE_ID)
    out, _ = capsys.readouterr()
    assert u"VenueId: 42, VenueName: Venue 42" in out


def test_add_numeric_column(capsys):
    snippets.add_numeric_column(INSTANCE_ID, DATABASE_ID)
    out, _ = capsys.readouterr()
    assert u'Altered table "Venues" on database ' in out
...
def test_query_data_with_numeric_parameter(capsys):
    ...
    out, _ = capsys.readouterr()
    assert u"VenueId: 4, Revenue: 35000" in out


def test_query_data_with_timestamp_parameter(capsys):
    snippets.query_data_with_timestamp_parameter(INSTANCE_ID, DATABASE_ID)
    out, _ = capsys.readouterr()
    assert u"VenueId: 4, VenueName: Venue 4, LastUpdateTime:" in out
    assert u"VenueId: 19, VenueName: Venue 19, LastUpdateTime:" in out
    assert u"VenueId: 42, VenueName: Venue 42, LastUpdateTime:" in out
