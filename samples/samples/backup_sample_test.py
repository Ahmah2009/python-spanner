...
def unique_backup_id():
    u""" Creates a unique id for the backup. """
...
def spanner_instance():
    ...
    instance_config = u"{}/instanceConfigs/{}".format(
        spanner_client.project_name, u"regional-us-central1"
    )
...
def test_restore_database(capsys):
    ...
    out, _ = capsys.readouterr()
    assert (DATABASE_ID + u" restored to ") in out
    assert (RESTORE_DB_ID + u" from backup ") in out
    assert BACKUP_ID in out


@pytest.mark.skip(
    reason=(
        u"failing due to a production bug"
        u"https://github.com/googleapis/python-spanner/issues/149"
    )
)
...
def test_cancel_backup(capsys):
    ...
    out, _ = capsys.readouterr()
    cancel_success = u"Backup creation was successfully cancelled." in out
    cancel_failure = (u"Backup was created before the cancel completed." in out) and (
        u"Backup deleted." in out
    )
